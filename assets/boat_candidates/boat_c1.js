// Boat Candidate 1: Angular Low-Poly Skiff
export default function (THREE) {
  const g = new THREE.Group();

  // Materials (matching style lock)
  ...
  trimMat.name = 'wood';
  const deckMat = new THREE.MeshStandardMaterial({ color: 0x4a6572, roughness: 0.9, flatShading: true });
  deckMat.name = 'wood';
  const metalMat = new THREE.MeshStandardMaterial({ color: 0x8d99ae, roughness: 0.4, metalness: 0.6, flatShading: true });
  metalMat.name = 'metal';
  const lanternMat = new THREE.MeshStandardMaterial({ color: 0xffd166, emissive: 0xffa500, emissiveIntensity: 1.5, flatShading: true });

  // 1. Hull Geometry (4m length: Z from -2 to +2, 1.5m width: X from -0.75 to +0.75, 0.8m height)
  ...
  // 3. Mast & Rear Fin (Silhouette Anchor)
